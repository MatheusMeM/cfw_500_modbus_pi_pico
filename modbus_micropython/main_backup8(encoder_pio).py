...
# Definir os pinos para sinais A e B
PIN_A = 16  # GPIO16 conectado ao sinal A do encoder (Black)
PIN_B = 17  # GPIO17 conectado ao sinal B do encoder (White)

# Programa PIO para o encoder (usando o código de Jamon Terrell)
@asm_pio()
def quadrature_encoder():
    wrap_target()
    label("start")
    # Espera até que CLK (PIN_A) vá para baixo
    wait(0, pin, 0)
    # Verifica o estado de DATA (PIN_B)
    jmp(pin, "decrement")    # Se DATA (PIN_B) estiver alta, decrementa
    # Incrementa o registrador X
    mov(x, x + 1)
    jmp("continue")
    label("decrement")
    # Decrementa o registrador X
    mov(x, x - 1)
    label("continue")
    # Espera até que CLK (PIN_A) vá para alto
    wait(1, pin, 0)
    # Verifica o estado de DATA (PIN_B)
    jmp(pin, "increment")    # Se DATA (PIN_B) estiver alta, incrementa
    # Decrementa o registrador X
    mov(x, x - 1)
    jmp("start")
    label("increment")
...
def read_encoder():
    ...
    # Força a execução de 'in_(x, 32)' para obter o valor atual
    sm.exec("mov(isr, x)")
    sm.exec("push()")
    # Lê o valor do registrador X
    if sm.rx_fifo():
        raw_value = sm.get()
        # Ajusta para inteiro com sinal de 32 bits
        if raw_value >= 0x80000000:
            raw_value -= 0x100000000
        encoder_position = raw_value
...
def main():
    global max_RPM, VERBOSE_LEVEL, encoder_position, sm
    ...
    try:
        ...
        # Criar a StateMachine para o quadrature encoder
        sm = StateMachine(1, quadrature_encoder, freq=1000000, in_base=pin_a, jmp_pin=pin_b)
        sm.active(1)

        # Inicializar o registrador x para zero
        sm.exec("set(x, 0)")
        ...
        while True:
            ...
            # Ler dados do encoder
            read_encoder()
            print_verbose(f"Encoder Position: {encoder_position}", 2)
        ...
        print_verbose(f"[ERROR] An error occurred: {e}", 1)
