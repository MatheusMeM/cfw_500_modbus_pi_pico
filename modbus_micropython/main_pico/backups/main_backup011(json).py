    ...
    try:
        if cmd == "start":
            ...
            if len(parts) >= 2:
                ...
                if VERBOSE_LEVEL >= 2:
                    print_verbose("[ERROR] Specify the speed in RPM.", 2)
        elif cmd == "read_speed":
            rpm = cfw500.read_current_speed()
            if rpm is not None and VERBOSE_LEVEL >= 2:
                print_verbose(f"[INFO] Current Speed: {rpm:.2f} RPM", 2)
            elif rpm is None and VERBOSE_LEVEL >= 2:
                print_verbose("[ERROR] Failed to read current speed.", 2)
        elif cmd == "status":
            ...
            if state is not None and VERBOSE_LEVEL >= 2:
                ...
                print_verbose("[ERROR] Failed to read inverter status.", 2)
        elif cmd == "reset_fault":
            cfw500.reset_fault()
            if VERBOSE_LEVEL >= 2:
                print_verbose("[ACTION] Fault reset command sent.", 2)
            ...
            if len(parts) >= 2:
                ...
                if level in [0, 1, 2, 3]:
                    VERBOSE_LEVEL = level
                    if VERBOSE_LEVEL >= 2:
                        print_verbose(f"[INFO] Verbosity level set to {VERBOSE_LEVEL}.", 2)
                else:
                    if VERBOSE_LEVEL >= 2:
                        print_verbose("[ERROR] Invalid verbosity level. Use a value between 0 and 3.", 2)
            else:
                if VERBOSE_LEVEL >= 2:
                    print_verbose("[ERROR] Specify the verbosity level (0 to 3).", 2)
            ...
            if len(parts) >= 2:
                ...
                if mode in ["step", "deg"]:
                    ...
                    if VERBOSE_LEVEL >= 2:
                        print_verbose(f"[INFO] Encoder output mode set to '{encoder_output_mode}'.", 2)
                else:
                    if VERBOSE_LEVEL >= 2:
                        print_verbose("[ERROR] Invalid encoder output mode. Use 'step' or 'deg'.", 2)
            else:
                if VERBOSE_LEVEL >= 2:
                    print_verbose("[ERROR] Specify the encoder output mode ('step' or 'deg').", 2)
        elif cmd == "calibrate":
            encoder_offset = encoder_position
            save_configuration()
            if VERBOSE_LEVEL >= 2:
                print_verbose("[ACTION] Encoder calibrated. Current position set as zero.", 2)
            ...
            if VERBOSE_LEVEL >= 2:
                print_verbose("[INFO] Exiting the program.", 2)
            return False  # Signal to exit
        elif cmd == "test":
            if VERBOSE_LEVEL >= 2:
                print_verbose("[INFO] Executing the default test sequence.", 2)
            cfw500.start_motor(1000)
            await asyncio.sleep(5)
            cfw500.stop_motor()
        else:
            if VERBOSE_LEVEL >= 2:
                print_verbose("[ERROR] Unrecognized command. Type 'help' to see the list of commands.", 2)
    except Exception as e:
        print_verbose(f"[ERROR] Exception occurred: {e}", 2)
    return True  # Continue running
